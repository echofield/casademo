...
import { getDemoConversion } from '@/lib/demo/presentation-data'
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isDemoMode) {
      return NextResponse.json(getDemoConversion(period))
    }
    ...
    const [purchasesResult, clientSignalsResult, sellersResult] = await Promise.all([
      dateFilter
        ? supabase.from('purchases').select('id, amount, source, seller_id, client_id').gte('purchase_date', dateFilter)
        : supabase.from('purchases').select('id, amount, source, seller_id, client_id'),
      supabase.from('clients').select('id, seller_signal'),
    ...
    sellers.forEach((seller) => {
      sellerNameMap[seller.id] = seller.full_name
    })

    const clientSignalMap: Record<string, string | null> = {}
    clients.forEach((client) => {
      clientSignalMap[client.id] = (client as any).seller_signal || null
    })
    ...
    PURCHASE_SOURCES.forEach((source) => {
      sourceMap[source.value] = { count: 0, revenue: 0 }
    })

    const sellerMap: Record<string, { total_sales: number; crm_sales: number; crm_revenue: number }> = {}
    sellers.forEach((seller) => {
      sellerMap[seller.id] = { total_sales: 0, crm_sales: 0, crm_revenue: 0 }
    })
    ...
    purchases.forEach((purchase) => {
      ...
      const source = (purchase.source || 'walk_in') as PurchaseSource
    ...
    clients.forEach((client) => {
      const signal = (client as any).seller_signal || null
      const signalKey = signal || 'null'
    ...
    const by_signal: SignalBreakdown[] = Object.entries(signalPurchaseMap)
      .map(([signal, data]) => ({
        ...
        conversion_pct: signalClientCounts[signal] > 0 ? Math.round((signalClientMap[signal].size / signalClientCounts[signal]) * 100) : 0,
        avg_basket: data.count > 0 ? Math.round(data.revenue / data.count) : 0,
      }))
    ...
    const response: ConversionResponse = { period, by_source, by_seller, by_signal, totals }
